...
function UserRegistration() {
  ...
  const SubmitOrder = async () => {
    if (!nameClient || !date) {
      ...
      new Date().toLocaleDateString().split("/").reverse().join("-") >= date
    ) {
    ...
    for (let i = 0; i < cart.length; i++) {
      const body = {
        product_id: cart[i].id_product,
        order_id: idOrder,
        product_price: cart[i].price_product,
        qty: cart[i].qty,
      };

      await axios
        .post("http://localhost:3003/orderProduct", body)
        .then((resp) => {
    ...
    for (let i = 0; i < cart.length; i++) {
      const body = {
        ...
        qty:
          cart[i].qty_stock - cart[i].qty === 0
            ? -1
            : cart[i].qty_stock - cart[i].qty,
      };

      await axios
        .put("http://localhost:3003/product", body)
        .then((resp) => {
